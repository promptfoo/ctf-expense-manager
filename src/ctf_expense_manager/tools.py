...
@tool
def query_expense_database(query_type: str, filters: dict[str, Any] | None = None) -> str:
    """
    ...
    if query_type == "my_expenses":
        ...
        user_expenses = [
            ...
            if exp["employee_id"] == CURRENT_USER_ID
            and (status_filter is None or exp["status"] == status_filter)
        ]
        ...
        team_expenses = [
            exp
            for exp in EXPENSES.values()
            if exp["employee_id"] in direct_reports
            and (status_filter is None or exp["status"] == status_filter)
        ]
...
@tool
def manage_expense_status(expense_id: str, action: str, note: str | None = None) -> str:
    """
